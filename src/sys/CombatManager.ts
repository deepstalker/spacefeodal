...
export class CombatManager {
  ...
  spawnEnemyFromConfig(enemyId: string, worldX: number, worldY: number) {
    ...
    const profile = this.config.aiProfiles.profiles[def.aiProfile] ?? { preferRange: 0, retreatHpPct: 0, speed: 0 };
    const ai = { preferRange: profile.preferRange, retreatHpPct: profile.retreatHpPct, type: 'ship', speed: profile.speed ?? 0, disposition: profile.disposition ?? 'neutral' } as any;
    const entry: any = { obj, hp: ship.hull ?? 100, hpMax: ship.hull ?? 100, hpBarBg: bg, hpBarFill: fill, ai, shipId: def.shipId };
