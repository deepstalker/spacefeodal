...
- **ЛКМ**: 
  - Выбор целей для боя (враги подсвечиваются кругом)
  - Назначение атаки на выбранную цель для выделенной группы оружия
...
- **P**: пауза/снятие с паузы (с конфигурируемой системой остановки)
- **Ctrl+Shift+D**: отладочная информация о системе паузы
- **Клавиши 1-9**: выбор/отмена выбора слотов оружия
- **` (тильда)**: выбрать/отменить выбор всех орудий
...
### Боевая система
- **Группы оружия**: возможность выбирать одно или несколько орудий и назначать им индивидуальные цели
- **Автоматический огонь**: при назначении цели на оружие, корабль автоматически ведёт огонь, как только цель оказывается в зоне досягаемости
- **Система оружия**: 6 типов вооружения с разными характеристиками (лазеры, пушки, ракеты, рэйлганы, плазма, flak)
- **ИИ противников**: сложное поведение, управляемое через профили (патрулирование, агрессия, бегство), с учётом фракционных отношений
...
- **NPC**: автономные корабли с поведением "планетарный торговец" или "пират-патрульный", управляемые префабами из `stardwellers.json`
...
- `sensors.radar_range` (игровые единицы): Дальность обнаружения целей
...
- **Explorer**: Быстрый (MAX_SPEED: 2.0), с высоким ускорением (ACCELERATION: 0.05) и хорошей манёвренностью (TURN_SPEED: 0.03)
- **Trader**: Медленный (MAX_SPEED: 0.5), но очень манёвренный (TURN_SPEED: 0.06)
- **Fighter**: Сбалансированный по скорости (MAX_SPEED: 1.2) и резкий (высокие ускорения)
...
| Оружие  | Урон | Скоростр. | Дальность | Скорость снаряда | Редкость  |
|---------|------|-----------|-----------|------------------|-----------|
| Laser   | 8    | 4/сек     | 500       | 700              | Uncommon  |
| Cannon  | 16   | 2/сек     | 700       | 500              | Rare      |
| Missile | 26   | 1/сек     | 1500      | 200              | Epic      |
| Railgun | 22   | 1.5/сек   | 650       | 900              | Legendary |
| Plasma  | 14   | 3/сек     | 520       | 550              | Rare      |
| Flak    | 6    | 5/сек     | 780       | 480              | Common    |

- `muzzleOffset.x/y` (пиксели): Смещение точки выстрела от центра корабля

### stardwellers.json, ai_profiles.json & factions.json - ИИ, NPC и Фракции

Эта система заменяет устаревший `enemies.json` и позволяет гибко настраивать NPC.

#### Префабы NPC (stardwellers.json)
- `prefabs.{prefabId}`: Определения "шаблонов" NPC.
- `shipId` (строка): Ссылка на тип корабля из `ships.json`.
- `aiProfile` (строка): Ссылка на профиль поведения из `ai_profiles.json`.
- `combatAI` (строка): Профиль боевого поведения (например, "coward" или "default_aggressive").
- `faction` (строка): Принадлежность к фракции из `factions.json`.
- `weapons` (массив строк): Список установленного оружия.
- **Примеры**:
  - `trader`: использует корабль "trader", поведение "planet_trader", фракцию "planet_traders".
  - `pirate`: использует корабль "fighter", поведение "patrol", фракцию "pirate" и вооружен лазером и пушкой.

#### Профили ИИ (ai_profiles.json)
- `profiles.{profileId}`: Определения моделей поведения.
- `behavior` (строка): Основной скрипт поведения (`"aggressive"`, `"static"`, `"planet_trader"`, `"patrol"`).
- `sensors.react.onFaction`: Определяет реакцию на другие фракции:
  - `confrontation`: атаковать (`attack`) или убегать (`flee`).
  - `ally`: игнорировать (`ignore`) или искать сопровождение (`seekEscort`).
  - `neutral`: игнорировать (`ignore`).
- `combat.retreatHpPct` (0.0-1.0): Порог здоровья для отступления.

#### Фракции и Отношения (factions.json)
- `factions.{factionId}`: Определяет фракцию.
- `relations`: Матрица отношений с другими фракциями.
  - `ally`: Союзник.
  - `neutral`: Нейтрал.
  - `confrontation`: Враг.
- **Пример**: Пираты (`pirate`) враждебны к игроку (`player`) и торговцам (`planet_traders`), но союзны друг другу.
...
- `weapons` (массив строк): Стартовое оружие
  - **По умолчанию**: ["laser", "cannon", "missile", "railgun", "plasma", "flak"]
